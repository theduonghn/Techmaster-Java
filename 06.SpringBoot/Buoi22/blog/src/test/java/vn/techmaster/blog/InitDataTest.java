...
public class InitDataTest {
    ...
    void save_image() {
        ...
        for (int i = 0; i < 150; i++) {
            ...
            Image image = Image.builder().url(faker.company().logo()).user(userRd).build();
    ...
    void save_blog() {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            String imageRd;
            if (images.size() > 0) {
                imageRd = images.get(rd.nextInt(images.size())).getUrl();
            } else {
                imageRd = null;
            }
            ...
            Blog blog = Blog.builder().title(title).slug(slugify.slugify(title)).description(faker.lorem().sentence(50))
                    .content(faker.lorem().sentence(100)).thumbnail(imageRd).categories(categoriesRd)
                    .status(rd.nextInt(2)).user(userRd).build();
